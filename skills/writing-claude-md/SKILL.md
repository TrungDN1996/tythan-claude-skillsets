...
description: Write, review, or trim a CLAUDE.md file for a Claude Code project. Covers structure, length limits, and progressive disclosure into linked docs. Use when the user asks to create, write, review, shorten, or improve a CLAUDE.md.
...
Write file as `CLAUDE.md` in project root (repo top-level dir, next to `.git`). Not in `.claude/`, not in subfolder. If root already got one, edit in place — don't create second copy elsewhere.
